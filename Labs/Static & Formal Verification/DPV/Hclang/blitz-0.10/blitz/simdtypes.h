...
public:
  ...
  {
#ifdef HECTOR
    //  Hector's pointers are fully symbolic, so casting them to integer
    //  returns a fully symbolic value. Returning true here basically says
    //  that Hector's pointers are always aligned.
    //
    return true; }
#else
...
#ifdef HECTOR
    //  Hector's pointers are fully symbolic, so casting them to integer
    //  returns a fully symbolic value. Returning 0 here basically says that
    //  Hector's pointers are always aligned.
    //
    return 0;
#else
