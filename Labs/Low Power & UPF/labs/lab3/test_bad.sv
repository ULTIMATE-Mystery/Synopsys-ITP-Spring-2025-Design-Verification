program automatic test(router_io.TB rtr_io, reset_io.TB rst_io);
  ...
  //Lab 3 - Task 5, Step 2
  //
  //import the Synopsys Low Power Message package SNPS_LP_MSG
  //ToDo
  import SNPS_LP_MSG::*;
  `endif
  ...
  initial begin
    ...
    begin
      ...
      repeat (20) @(rtr_io.drvClk);
      //cycle power on output 2
      op_power_cycle_bad(2, 60000);
      `endif
    end
    join
  ...
  task op_power_cycle_bad(int port_no, time off_time);
  `ifdef UPF
    static int off_oport[$];
  if (TRACE_ON) $display("[TRACE]%t :%m", $realtime);
  //wait for output port to be idle
  ...
  @(rtr_io.drvClk);
    router_test_top.dut.pwr.opwrcon_iso_n[port_no] = 1'b1; //unisolate
  off_oport = off_oports.find_first_index(port) with (port == port_no);
  if (TRACE_ON) $display("[NOTE]%t off_oports :%p", $realtime, off_oports);
  if(off_oport.size()) off_oports.delete(off_oport[0]);
  off_oport.delete();
  `endif
  endtask: op_power_cycle_bad
